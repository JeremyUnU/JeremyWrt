## `【_Jeremy_无Docker带IPv6固件】（6.6内核）`
#### 🚩 源码拉取`coolsnowwolf` `Lienol` `immortalwrt` `x-wrt` `openwrt` 自行选择编译
- _Jeremy_适配OTA自动升级のX86固件
- 默认IP地址：`10.10.10.2`
- 账户：`root`   密码：`空`
 
#### 🚩 点击下表中 [![](https://img.shields.io/badge/下载-链接-blueviolet.svg?style=flat&logo=hack-the-box)](https://github.com/JeremyUnU/JeremyWrt/releases) 即可跳转到该设备固件下载页面
